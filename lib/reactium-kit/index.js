...
module.exports = config => {
    ...
    catch (err) { rconfg = {version: '2.3.7'}; }
    ...
    const getManifest = () => {
        ...
        let fname = (semver.lt(rconfg.version, '2.3.7')) ? 'manifest' : 'index';

        let p = path.normalize(`${base}/src/app/toolkit/${fname}.js`);
        ...
        try {
            ...
            let f = fs.readFileSync(p, 'utf8');
                ...
                f = f.replace(/\"/g, "'");
                f = f.replace(/\srequire(.*?)default/g, ' "require$1default"');
                f = beautify(f);

            let t   = path.normalize(`${base}/src/app/toolkit/.tmp/${fname}.js`);
            ...
            log(prefix, chalk.red('re:kit error:'), `toolkit '${fname}.js' file not found`, err);
        }
    ...
    const writeManifest = ({ manifest, bar, backupOnly = false }) => {
        ...
        let b = path.normalize(`${base}/src/app/toolkit/.backup/${Date.now()}.${fname}.BACKUP`);
        ...
        // Create a backup of the current toolkit/index.js file.
        fs.copySync(p, b);
        bar.interrupt(`  Created /toolkit/${fname}.js backup`);
        bar.tick();
        ...
        // Update the toolkit/index.js file
        fs.ensureFileSync(t);
        fs.writeFileSync(t, f, 'utf8');
        fs.copySync(t, p);
        bar.interrupt(`  Updated /toolkit/${fname}.js`);
        bar.tick();
    ...
    const writeStyle = ({ group, component, name, overwrite }) => {
        ...
        let stylePath    = `${base}/src/app/toolkit/${group}/${component}/${name}`;
    ...
    const element = {
        ...
        create: (type, opt) => {
            ...
            manifest.menu[group]['elements'][id] = {
                ...
                dna       : `/toolkit/${group}/${component}`,
                component : `require("appdir/toolkit/${group}/${component}").default`,
                readme    : `require("appdir/toolkit/${group}/${component}/readme").default`,
            };
            ...
            let componentFileName = `${base}/src/app/toolkit/${group}/${component}/index.js`;
            writeComponent({ bar, name: component, file: componentFileName });

            // Write the readme file (1 tick)
            let readmeFileName = `${base}/src/app/toolkit/${group}/${component}/readme.js`;
    ...
    const style = {
        schema: ({ type, opt }) => {
            ...
            // Delete the style prompt from the schema
            delete output.properties['style'];
