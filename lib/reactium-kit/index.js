...
module.exports = config => {
    ...
    const getManifest = () => {
        ...
        try {
            ...
            let t   = path.normalize(`${base}/tmp/reactium-kit/manifest.js`);
    ...
    const writeManifest = ({ manifest, bar }) => {
        ...
        let tmp = path.normalize(`${base}/tmp`);
        fs.removeSync(tmp);
    ...
    const writeStyle = ({ id, group, component, name, overwrite }) => {
    ...
    const page = {
        ...
        create: (type, opt) => {
            ...
            let { id, group, name, overwrite, route, target, index = -1 } = opt;
    ...
    const element = {
            ...
            if (op.has(menu, 'elements')) {
                ...
                if (max > 0) {
                    ...
                    output.properties['index'] = {
                        ...
                        message     : `Menu position must be a value number ${range}]`,
                    }
                }
            }
        ...
        create: (type, opt) => {
            ...
            let { id, group, name, overwrite, index = -1 } = opt;
            ...
            let keys = Object.keys(manifest.menu[group]['elements']);
                keys.splice(index, 0, id);
                keys.forEach((k) => { menu[group]['elements'][k] = manifest.menu[group]['elements'][k]; });
    ...
    const group = {
        schema: ({ type, opt }) => {
            ...
            if (max > 0) {
                ...
                output.properties['index'] = {
                    ...
                    default     : max,
                    description : chalk.yellow(`Menu Index ${range}:`),
                    message     : `Menu index must be a value number ${range}]`,
                }
            }
        ...
        create: (type, opt) => {
            ...
            let { group:id, index = -1, name, overwrite } = opt;
    ...
    const style = {
        ...
        create: (type, opt) => {
            ...
            if (writeStyle({ id, group, component, name, overwrite }) === false) {
