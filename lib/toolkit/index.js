...
module.exports = config => {
    ...
    const style = {
        ...
        create: (opt, spinner) => {
            ...
            let theme     = opt['theme'] || config.toolkit.theme;
            let spath     = path.normalize(base + config.toolkit.src + '/assets/toolkit/styles/themes/' + theme + '/');
            let sfile     = spath + '_' + name + '.scss';
            let sspath    = path.normalize(base + config.toolkit.src + '/assets/toolkit/styles/themes/' + theme + '/');
    ...
    const template = {
        ...
        create: (opt) => {
            ...
            let tpath      = base + '/' + config.toolkit.src + '/views/templates/';
    ...
    const material = {
        ...
        create: (type, opt) => {
            ...
            if (opt.hasOwnProperty('lib')) {
                config.toolkit.src  += '/lib/' + opt.lib;
            }
            ...
            let dna        = name;
            let mpath      = base + config.toolkit.src + '/materials/' + id;
            ...
            let vpath      = base + '/' + config.toolkit.src + '/views/';
            ...
            if (type === 'catalyst') {
                ...
                let jpath    = base + '/' + config.toolkit.src + '/catalyst/components';
            ...
            let css    = opt.style;
            css       = (css.length > 0) ? css.toLowerCase() : 'n';
            if (css !== 'n') {
    ...
    const install = {
        ...
        start: (opt) => {
            ...
            // Download the most recent version of toolkit
            request(config.toolkit.install)
        ...
        complete: () => {
            ...
            log(prefix, 'run `arcli kit:launch` to start the dev environment');
        }
    };

    const launch = (opt) => {
        let spinner = ora({
            text       : 'Launching Toolkit...',
            spinner    : 'dots',
            color      : 'green'
        });
        ...
        let msg     = 'Running Toolkit: Press '+chalk.green('ctrl + c')+' to exit  ';
        ...
        process.on('SIGINT', function () {
            gulp.kill();
            spinner.succeed('Toolkit terminated\n');
            process.exit();
        });
    };
