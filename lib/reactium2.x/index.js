...
module.exports = config => {
    ...
    const types   = ['function', 'class', 'actions', 'types', 'reducers', 'services', 'route', 'style', 'state'];
    ...
    const update = {
        ...
        prompt: (opt) => {
            let schema = {
                properties: {
                    version: {
                        description     : chalk.yellow('Update command works with Reactium >=2.3.x. Are you sure you have a compatible version?  (Y/N):'),
                        before          : (val) => {
                            return (String(val).toLowerCase() === 'y');
                        }
                    },
        ...
        start: (type, opt) => {
            if (opt.confirm !== true || opt.version !== true) {
