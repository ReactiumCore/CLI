...
module.exports = config => {
    ...
    const types   = ['function', 'class', 'actions', 'types', 'reducers', 'services', 'route', 'style'];
    ...
    const styleGen = {
        ...
        create: (type, opt) => {
            ...
            if (fs.existsSync(path.join(base, entries))) {
                ...
                earr.pop();
                let prefix = file.replace(earr.join(path.sep), '');
                prefix = prefix.replace(fname, '');
                prefix = prefix.replace(/\W/, '');

                let mcont = fs.readFileSync(path.join(base, entries), 'utf-8');

                mcont += '\n\n';
                mcont += `@import '${prefix}${opt.name}';`;
