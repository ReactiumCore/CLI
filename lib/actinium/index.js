...
module.exports = config => {
    ...
    const generate = (type, opt) => {
        ...
        if (validType(type) !== true) {
            log(prefix, chalk.red('act:gen error:'), `<type> must be ${types.join(',')}`);
            return;
        }
    ...
    const install = {
        ...
        init: (opt) => {
            ...
            if (contents.length > 0 && !opt.overwrite) {
                ...
                log(chalk.red('[actinium] install error:'), '`cd` into an empty directory or specify `--overwrite true`');
        ...
        restore: (opt) => {
            let params = {
                root: dirname + '/lib/actinium/install',
                uri: opt.db,
                parser: 'json',
                drop: true,
                callback: function () {
        ...
        complete: () => {
            ...
            log('\n' + prefix, 'Run `arcli act:launch` to start the development instance\n');
            process.exit();
        }
    };
