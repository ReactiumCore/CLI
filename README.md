...
> Beware: \_Application specific commands are only suggested if you're contributing to framework core. In modern Reactium projects, core commands are in the `reactium_modules/@atomic-reactor/reactium-core/.cli/commands/` directory.
...
> _Actinium looks in the `~/src/app/cloud` directory for .js files and loads them as cloud functions._ > _You can use the `cloud/` shortcut when specifying the destination. Example: `$ reactium element --destination 'cloud/my/function` will put the cloud function in the `~/src/app/cloud/my/function` directory._
