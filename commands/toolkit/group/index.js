...
const CONFIRM = ({ props, params, message = 'Proceed?' }) => {
    ...
    return new Promise((resolve, reject) => {
        prompt.get({
            properties: {
                confirmed: {
                    description: `${chalk.white(message)} ${chalk.cyan('(Y/N):')}`,
                    type: 'string',
                    required: true,
                    pattern: /^y|n|Y|N/,
                    message: ` `,
    ...
    return {
        properties: {
            ...
            overwrite: {
                ...
                pattern: /^y|n|Y|N/,
                message: '',
                description: `${chalk.white('Overwrite existing group?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => {
                    try {
                        const id = prompt.override['id'] || prompt.history('id').value;
            ...
            menuOrder: {
                description: `${chalk.white('Menu Order')} ${chalk.cyan(`[0-${groupsLength}]:`)}`,
    ...
    return {
        properties: {
            id: {
                ...
                description: `${chalk.white('Group:')} ${groupList}\n    ${chalk.white('Select:')}`,
...
const ACTION_REMOVE = ({ opt, props }) => {
    ...
    prompt.get(schema, (err, input) => {
        ...
        const warning = `Are you sure you want to remove the ${chalk.cyan(params.id)} group?`;

        CONFIRM({ props, params, message: warning }).then(() => {
            console.log('');
...
const ACTION_UPDATE = ({ opt, props }) => {
    ...
    new Promise((resolve, reject) => {
        ...
        message(`The ${chalk.cyan(params.id)} group will be updated with the following options:`);
        const preflight = { ...input };
...
const COMMAND = ({ program, props }) => program.command(NAME)
    ...
    .option('-o, --overwrite [overwrite]', 'Overwrite existing group. Beware this will remove all children of the group.')
