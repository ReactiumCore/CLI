...
module.exports = (spinner) => {
    ...
    const generate = ({
        action, params, props, templateFile, fileName
    }) => {
        const { cwd } = props;
    ...
    return {
        backup: ({ action, params, props }) => {
            ...
            const backupZip = path.normalize(`${backupDir}/${now}.${group}.${name}.zip`);
        ...
        element: ({ action, params, props }) => generate({
            action, params, props, templateFile: 'component', fileName: 'index.js',
        }),

        readme: ({ action, params, props }) => generate({
            action, params, props, templateFile: 'readme', fileName: 'readme.js',
        }),
        ...
        manifest: ({ action, params, props }) => {
            ...
            if (!isNaN(menuOrder)) {
                ...
                keys.forEach(key => {
                    elements[key] = op.get(manifest, `menu.${group}.elements.${key}`, {});
                });
