...
const {
    formatImport,
    topLevelStyles
} = require('../../reactium/style');

const {
    formatName,
    formatRoute
} = require('../../reactium/component');
...
const mapStyleSheet = ({ output, input, props }) => {
    ...
    if (op.get(output, 'stylesheet', false) === true) {
        const dest = path.normalize(`${cwd}/src/app/toolkit/${output.group}/${output.name}`);
        const stylesheetFile = path.normalize(path.join(
            dest,
            '_style.scss'
        ));

        const importString = output.inject.map(filepath => path.relative(
            filepath,
            stylesheetFile
        ).replace(/^\..\//, '').replace('_style.scss', 'style'));
...
const CONFIRM = ({ props, params, description }) => {
    ...
    description = description || `${chalk.white('Proceed?')} ${chalk.cyan('(Y/N):')}`;
...
const CONFORM_UPDATE = ({ props, input }) => {
    ...
    const groupElements = Object.keys(op.get(m, `menu.${output.prev.group}.elements`, {}));
    ...
    Object.entries(input).forEach(([key, val]) => {
        switch(key) {
            case 'name':
                ...
                output.prev[key] = formatName(path.basename(output.prev.element.dna));
                break;
    ...
    // component path
    output.new['component'] = `appdir/toolkit/${output.new.group}/${output.new.name}`;
...
const SCHEMA_ACTION = ({ props }) => {
    ...
    const actionList = actions.map((item, index) => listItem({
        item, index, padding: String(actions.length).length
    })).join('');
    ...
    return {
        properties: {
            action: {
                ...
                description: `${chalk.white('Action:')} ${actionList}\n    ${chalk.white('Select:')}`,
...
const SCHEMA_CREATE = ({ props }) => {
    ...
    const styleList = styles.map((item, index) => listItem({
        item, index, padding: String(styles.length).length
    })).join('');

    const groups = getGroups({ props });
    const groupList = groups.map((item, index) => listItem({
        item, index, padding: String(groups.length).length
    })).join('');
    ...
    return {
        properties: {
            ...
            group: {
                required: true,
                message: 'Select the menu group or enter the name of a new group.',
                description: `${chalk.white('Menu Group:')} ${groupList}\n    ${chalk.white('Select:')}`,
            ...
            overwrite: {
                ...
                pattern: /^y|n|Y|N/,
                message: '',
                description: `${chalk.white('Overwrite existing component?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => {
                    try {
                        const group = prompt.override['group'] || prompt.history('group').value;

                        let name = prompt.override['name'] || prompt.history('name').value;
                            name = formatID(name);

                        return (op.get(m, `menu.${group}.elements.${name}`, false) !== false);
            ...
            documentation: {
                pattern: /^y|n/i,
                default: 'Y',
                description: `${chalk.white('Documentation?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => canOverwrite,
                before: (val) => {
                    return (String(val).toLowerCase() === 'y');
                }
            ...
            code: {
                pattern: /^y|n/i,
                default: 'Y',
                description: `${chalk.white('Show Code?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => canOverwrite,
                before: (val) => {
                    return (String(val).toLowerCase() === 'y');
                }
            ...
            dna: {
                pattern: /^y|n|Y|N/,
                default: 'Y',
                description: `${chalk.white('Show DNA?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => canOverwrite,
                before: (val) => {
                    return (String(val).toLowerCase() === 'y');
                }
            ...
            stylesheet: {
                ...
                description: `${chalk.white('Stylesheet?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => canOverwrite,
                before: (val) => {
                    return (String(val).toLowerCase() === 'y');
                }
            ...
            inject: {
                ...
                description: `${chalk.white('Import stylesheet to:')} ${styleList}\n    ${chalk.white('Select:')}`,
                ask: () => {
                    try {
                        return prompt.override['stylesheet']
...
const SCHEMA_ELEMENT = ({ props, group }) => {
    ...
    const elementList = elements.map((item, index) => listItem({
        item, index, padding: String(elements.length).length
    })).join('');
    ...
    return {
        properties: {
            ID: {
                ...
                description: `${chalk.white('Element:')} ${elementList}\n    ${chalk.white('Select:')}`,
                before: (input) => {
                    if (!isNaN(input)) {
...
const SCHEMA_UPDATE = ({ props, params }) => {
    ...
    const { type, label, hideDna = false, hideCode = false, hideDocs = false } = element;
    const { cwd, prompt, config } = props;
    ...
    return {
        properties: {
            ...
            menuOrder: {
                default: menuOrder,
                description: `${chalk.white('Menu Order:')} ${chalk.cyan(`[0-${groupElements.length - 1}]`)}`,
            },
            documentation: {
                pattern: /^y|n/i,
...
const ACTION_REMOVE = ({ opt, props }) => {
    ...
    const prom = new Promise((resolve, reject) => {
        ...
        const description = `${chalk.white('Are you sure you want to delete')} ${chalk.cyan(`${group}.${ID}`)} ${chalk.white('element?')} ${chalk.cyan('(Y/N):')}`;

        params['key']         = key;
        params['remove']      = true;
        params['element']     = element;
        params['name']        = path.basename(element.dna);
        params['destination'] = path.normalize(`${cwd}/src/app/${element.dna}`);
