...
const formatImport = (val, props) => {
    ...
    val = val.replace(/[^0-9\s]/g, '').replace(/\s\s+/g, ' ').trim();
    val = _.compact(val.split(' ').map(v => {
...
const topLevelStyles = ({ props }) => {
    ...
    return globby([`${cwd}/src/**/*.scss`])
    .filter(file => (String(path.basename(file)).substr(0, 1) !== '_'))
};
...
const CONFORM = ({ input, props }) => {
    ...
    // Set the file path
    output['filepath'] = path.normalize(path.join(
        output.destination,
        output.filename
    ));
    ...
    output['ext'] = ext;
    output['importString'] = output.inject.map(filepath => path.relative(
        filepath,
        output.filepath
    )
    .replace(/^\..\//, '')
...
const HELP = () => {
    ...
    console.log('Example:');
    console.log('');
    console.log(`  arcli style --filename '_fubar.scss' --destination '~/src/assets/style' --overwrite`);
    console.log('');
};
    ...
    return {
        properties: {
            ...
            overwrite: {
                ...
                pattern: /^y|n|Y|N/,
                message: '',
                description: `${chalk.white('Overwrite existing stylesheet?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => {
                    try {
                        let dest = prompt.override['destination'] || prompt.history('destination').value;
                            dest = formatDestination(dest, props);

                        let file = prompt.override['filename'] || prompt.history('filename').value;
            ...
            inject: {
                pattern: /[0-9\s]/,
                description: `${chalk.white('Import to:')} ${styles}\n    ${chalk.white('Select:')}`,
                required: true,
...
const COMMAND = ({ program, props }) => program.command(NAME)
    ...
    .option('-d, --destination [destination]', 'Path where the stylesheet is saved.')
    .option('-f, --filename [filename]', 'File name.')
    .option('-o, --overwrite [overwrite]', 'Overwrite the existing stylesheet.', false)
    .on('--help', HELP);
