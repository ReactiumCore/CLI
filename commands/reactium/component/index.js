...
const formatRoute = (val) => {
    ...
    val = val.toLowerCase();
    val = val.replace(special, c => specialReplace.charAt(specialChars.indexOf(c)));
...
const CONFORM = ({ input, props }) => {
    ...
    Object.entries(input).forEach(([key, val]) => {
        switch(String(key).toLowerCase()) {
            ...
            case 'reducers':
            case 'services':
                output[key] = (typeof val === 'string' && val.toUpperCase() === 'Y') ? true : val;
                break;
...
const SCHEMA = ({ props }) => {
    ...
    const typeSelections = types.map((type, index) => {
    ...
    const styles = styleList.map((file, index) => {
        index += 1;
    ...
    return {
        properties: {
            ...
            overwrite: {
                ...
                ask: () => {
                    try {
                        let dir  = prompt.override['name'] || prompt.history('name').value;
            ...
            type: {
                ...
                description: `${chalk.white('Type:')} ${typeSelections}\n    ${chalk.white('Select:')}`,
                before: (val) => {
                    if (!isNaN(val)) {
            ...
            route: {
                ...
                ask: () => {
                    ...
                    try {
                        type = prompt.override['type'] || prompt.history('type').value;
                    } catch (err) {
                        type = 'class';
                    }
            ...
            actions: {
                pattern: /^y|n|Y|N/,
                default: 'Y',
                description: `${chalk.white('Actions?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => {
                    try {
                        return prompt.override['redux'] || prompt.history('redux').value && overwritable(prompt);
                    } catch (err) {
                        return false;
                    }
                },
            ...
            actionTypes: {
                pattern: /^y|n|Y|N/,
                default: 'Y',
                description: `${chalk.white('Actions Types?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => {
                    try {
                        return prompt.override['redux'] || prompt.history('redux').value && overwritable(prompt);
                    } catch (err) {
            ...
            reducers: {
                pattern: /^y|n|Y|N/,
                default: 'Y',
                description: `${chalk.white('Reducers?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => {
                    try {
                        return prompt.override['redux'] || prompt.history('redux').value && overwritable(prompt);
                    } catch (err) {
            ...
            services: {
                pattern: /^y|n|Y|N/,
                default: 'Y',
                description: `${chalk.white('Services?')} ${chalk.cyan('(Y/N):')}`,
                ask: () => overwritable(prompt),
                before: (val) => {
                    return (String(val).toLowerCase() === 'y');
                }
            ...
            inject: {
                pattern: /[0-9\s]/,
                description: `${chalk.white('Import stylesheet to:')} ${styles}\n    ${chalk.white('Select:')}`,
                required: true,
                message: 'Select a number or list of numbers. Example: 1 2 3',
                ask: () => {
                    try {
                        return prompt.override['stylesheet'] || prompt.history('stylesheet').value && overwritable(prompt);
                    } catch (err) {
                        return false;
                    }
                }
...
const ACTION = ({ opt, props }) => {
    ...
    if (op.get(opt, 'reduxAll', false) === true) {
        ...
        reduxParams.forEach(k => { ovr[k] = true; });
    }
