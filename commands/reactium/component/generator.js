...
module.exports = ({ params, props }) => {
    ...
    if (!op.get(params, 'redux', false)) {
        const noredux = ['redux', 'subclass', 'actions', 'actionTypes', 'reducers', 'state'];
        noredux.forEach((exclude) => {
            delete actions[exclude];
        });
    }

    const excludes = ['actions', 'actionTypes', 'reducers', 'route', 'services'];
    ...
    if (op.get(params, 'stylesheet', false)) {
        try {
            ...
            actions['stylesheet'] = ({ action, params, props }) => ActionSequence({
