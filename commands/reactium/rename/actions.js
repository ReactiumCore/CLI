...
module.exports = (spinner) => {
    ...
    const backupfile = ({ params, props, file }) => {
        ...
        const backupFilePath = path.normalize(`${cwd}/.BACKUP/${now}.${filename}`);
    ...
    return {
        ...
        filename: ({ action, params, props }) => {
            ...
            const promises = files.filter(file => regex.test(
                path.basename(file)
            )).map(file => {

                const dirname  = path.dirname(file);
                const filename = String(path.basename(file)).replace(from, to);
