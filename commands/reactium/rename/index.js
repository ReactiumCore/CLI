...
const SEARCH = ({ props, params }) => {
    ...
    : new Promise((resolve, reject) => {
        ...
        prompt.get({
            properties: {
                directory: {
                    description: `${chalk.white('Component Directory:')} ${selections}\n    ${chalk.white('Select:')}`,
                    type: 'string',
                    required: true,
                    message: ` please make a selection`,
                    before: (val) => {
                        val = val.replace(/[^0-9\s]/g, '').replace(/\s\s+/g, ' ').trim();
                        const index = Number(String(val).replace(/[^0-9]/gi)) - 1;
        ...
        switch(key) {
            case 'directory':
                if (val) {
                    ...
                    output['destination'] = formatDirectory({ val, props, name: to });
                }
                break;
...
const ACTION = ({ opt, props }) => {
    ...
    prompt.get(schema, (err, input) => {
        ...
        SEARCH({ props, params: input }).then(directory => {
